...
{
  if(!empty($_POST['txt_name']) && !empty($_POST['txt_date_end'])
    && !empty($_POST['txt_weigh_end'])&& !empty($_POST['txt_lost'])
    && !empty($_POST['id']))
  {
    $id = $conn->escape_string($_POST['id']);
    $room = $conn->escape_string($_POST['txt_name']);
    $date_end = $conn->escape_string($_POST['txt_date_end']);
    $weigh_end = $conn->escape_string($_POST['txt_weigh_end']);
    $lost = $conn->escape_string($_POST['txt_lost']);
    $status = "เก็บเกี่ยวแล้ว";
    $sql = "update start_end set room='$room', date_end='$date_end', weigh_end=$weigh_end, lost=$lost, status='$status'  where id='$id'";
